<script language="javascript" type="text/javascript">
	...
	$("#xoahet").click(function(){
		...
		if (hoi==true) document.location = "index.php?com=contact&act=delete&listid=" + listid+"&type=<?= $_GET['type'] ?>";
	});
	});
	...
	{	
		...
		window.location ="index.php?com=contact&act=man&key="+a+"&type=<?= $_GET['type'] ?>";	
		return true;
	}	
</script>
...
<div class="control_frm">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=contact&act=man&type=<?=$_GET['type']?>"><span>Danh sách liên hệ</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    <thead>
      <tr class="tt_table">
        ...
        <td width="150" class="<?=(!$config_s['sanpham'])?'none':''?>">Sản phẩm</td>
    ...
    <tbody>
    <form name="f" id="f" method="post" action="index.php?com=contact&act=savestt<?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>">
       ...
       <td>
        ...
        <td align="center " class="<?=(!$config_s['ten'])?'none':''?>">
          	<a href="index.php?com=contact&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&type=<?=$_GET['type']?>" class="tipS SC_bold">
                  <?=$items[$i]['ten']?>
            </a>
        </td>
      <td width="150" class="<?=(!$config_s['dienthoai'])?'none':''?>"> <?=$items[$i]['dienthoai']?></td>
       <td width="150 " class="<?=(!$config_s['sanpham'])?'none':''?>"><?=$items[$i]['sanpham']?></td>
        ...
        <td class="actBtns">
            <a href="index.php?com=contact&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&type=<?=$_GET['type']?>" title="" class="btn btn-info" original-title="Sửa contact"><i class="fa fa-pencil"></i> Sửa</a>
           
            <a href="index.php?com=contact&act=delete&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&type=<?=$_GET['type']?>" onClick="if(!confirm('Xác nhận xóa dữ liệu. Việc xóa dữ liệu sẽ không khôi phục được, Bạn cần cân nhắc và chịu trách nhiệm cho thao tác này !')) return false;" title="" class="btn btn-danger" original-title="Xóa bài viết"><i class="fa fa-times"></i> Xóa</a>      
             </td>
      </tr>
         <?php } ?>
</form> 
