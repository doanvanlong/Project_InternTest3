...
<div id="div_fixed">
  ...
  <a href="index.php?com=news&act=man_danhmuc<?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?><?php if($_REQUEST['type']!='') echo'&type='.$_REQUEST['type'];?>" onClick="if(!confirm('Bạn có muốn thoát không ? ')) return false;" title="" class="btn btn-warning" original-title="Thoát"><i class="fa fa-warning"></i>&nbsp;Thoát</a>
</div>
...
<div class="clear"></div>
 <div id="info" class="tab_content">
    <div class="formRow <?= (!$config_s['icon'])?'none':'' ?>">
      <label>Tải icon:</label>
      <div class="formRight">
              <input type="file" id="file1" name="file1" />
        <img src="./images/question-button.png" alt="Upload hình" class="icon_question tipS" original-title="Tải hình ảnh (ảnh JPEG, GIF , JPG , PNG)">
        <div class="note"> <?=$config_s['size_icon']?>  <?=_format_duoihinh_l?> </div>
      </div>
      <div class="clear"></div>
    </div>
   <?php if($_GET['act']=='edit_danhmuc'){?>
    <div class="formRow <?= (!$config_s['icon'])?'none':'' ?>">
      <label>Hình Icon Hiện Tại :</label>
      <div class="formRight">
      <div class="mt10"><img src="<?=_upload_tintuc.$item['icon']?>"  width="100px" alt="NO PHOTO" /></div>
      </div>
      <div class="clear"></div>
    </div>
    <?php } ?>
                  ...
                  <div class="photoUpload-zone">
                    ...
                    <div class="photoUpload-detail" id="photoUpload-preview"><img class="rounded" src="<?=_upload_tintuc.$item['photo']?>" onerror="src='images/noimage.png'" alt="Alt Photo"/><div class="none">Hình hiện tại</div>
                      ...
                      <div class="detailImages">
                        ...
                        <div class="btn"><div class="removeInput photoRemoveInput" data-id="<?=$item["id"]?>" data-photo="<?=$item["photo"]?>" data-format="<?=_upload_tintuc?>" data-table="news_danhmuc" data-field="photo"><a class="icon-jfi-trash jFiler-item-trash-action"></a></div></div>
