<div class="wfooter-help">
	<div class="footer-help">
		<div class="footer-help-svg-icon">
			<svg id="next-help-circle">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
					<circle cx="10" cy="10" r="9" fill="#F7F7F7" ></circle>
					<path d="M10 0c-5.514 0-10 4.486-10 10s4.486 10 10 10 10-4.486 10-10-4.486-10-10-10m0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8m0-4c-.553 0-1 .447-1 1 0 .553.447 1 1 1 .553 0 1-.447 1-1 0-.553-.447-1-1-1m0-10c-1.654 0-3 1.346-3 3 0 .552.447 1 1 1 .553 0 1-.448 1-1 0-.551.448-1 1-1s1 .449 1 1c0 .322-.149.617-.409.808-1.011.74-1.591 1.808-1.591 2.929v.263c0 .553.447 1 1 1 .553 0 1-.447 1-1v-.263c0-.653.484-1.105.773-1.317.768-.564 1.227-1.468 1.227-2.42 0-1.654-1.346-3-3-3"></path>
				</svg>
			</svg>
		</div>
		<div class="footer-help-txt">Bạn có thể xem thêm hướng dẫn sử dụng trang quản trị <a href="<?=$company['link_huongdan']?>" target="_blank">tại đây</a>.</div>	
		<div class="clear"></div>
	</div>
</div>



<style>
	#next-help-circle{
		fill:#3A596D;
		color: #F7F7F7;
		width: 21px;
		height: 21px;
	}
	.wfooter-help{
		text-align: center;

	}
	.footer-help{
		display: inline-flex;
		margin: 20px auto;
		text-align: center;
		border: 1px solid #dfe4e8;
		border-radius: 999px;
		padding:1rem 1.6rem 0.7rem 1.6rem;
		color: #212b35;
	}
	.footer-help-svg-icon{
		float: left;
		margin-right: 10px;
	}
	
	.footer-help-txt{
		float: left;
	}
	.footer-help-txt a{
		display: inline-block;
	}
</style>