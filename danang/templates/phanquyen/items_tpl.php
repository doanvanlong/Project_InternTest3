...
<div class="control_frm" style="margin-top:0;">
  	<div id="div_fixed">
    	  <input type="button" class="btn btn-info" value="Thêm" onclick="location.href='index.php?com=phanquyen&act=add'" />
 ...
 <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
	...
	<tbody>
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
        ...
        <td class="actBtns">
            <a href="index.php?com=phanquyen&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" title="" class="btn btn-info" original-title="Sửa danh mục"><i class="fa fa-pencil"></i> Sửa</a>
           <a href="index.php?com=phanquyen&act=delete&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" onClick="if(!confirm('Xác nhận xóa <?=$items[$i]['ten']?>')) return false;" title="" class="btn btn-danger" original-title="Xóa bài viết"><i class="fa fa-times"></i> Xóa</a> 
        </td>
      </tr>
           <?php } ?> 
    </tbody>
