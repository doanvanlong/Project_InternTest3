...
function tinhtrang($i=0)
	{
		...
		$str='
			<select id="id_tinhtrang" name="id_tinhtrang" class="main_font main_select select_danhmuc chzn-select">					
			';
...
function hinhthucgiaohang($i=0)
	{
		...
		$stmt=mysql_query($sql);
		$str='
			<select id="hinhthucgiaohang" name="hinhthucgiaohang" class="main_font main_select select_danhmuc chzn-select">					
			';
		while ($row=@mysql_fetch_array($stmt)) 
		{
			if($row["id"]==$i)
...
function get_httt()
	{
		$sql="select * from table_news where type='hinh-thuc-thanh-toan' and hienthi=1 order by stt,id desc";
		$stmt=mysql_query($sql);
		$str='
			<select id="httt" name="httt" class="main_select select_danhmuc chzn-select">
...
function get_thanhpho_item() 
	{
		...
		$stmt=mysql_query($sql);
		$str='
			<select id="thanhpho_item" name="thanhpho_item" class="main_select select_danhmuc chzn-select w-100" >
...
function get_thanhpho()
	{
		...
		$stmt=mysql_query($sql);
		$str='
			<select id="thanhpho" name="thanhpho" class="main_select select_danhmuc chzn-select">
...
function get_phuong()
	{
		...
		$stmt=mysql_query($sql);
		$str='
			<select id="phuong" name="phuong" class="main_select select_danhmuc chzn-select" >
...
<form name="supplier" id="validate" class="form" action="index.php?com=order&act=save" method="post" enctype="multipart/form-data">
	<div id="div_fixed">
	  ...
	  <a href="index.php?com=order&act=man<?php if($_REQUEST['curPage']!='') echo'&curPage='.$_REQUEST['curPage'];?><?php if($_REQUEST['type']!='') echo'&type='.$_REQUEST['type'];?>" onClick="if(!confirm('Bạn có muốn thoát không ? ')) return false;" title="" class="btn btn-warning" original-title="Thoát"><i class="fa fa-warning"></i>&nbsp;Thoát</a>
	</div>
	<div class="widget mt-10">
		...
		<div class="formRow">
			...
			<div class="formRight">
            <input type="text" name="madonhang" title="Mã đơn hàng"  readonly="readonly" id="madonhang" class="tipS validate[required]" value="<?=@$item['madonhang']?>" />  
			</div>
			<div class="clear"></div>
		</div>	
        ...
        <div class="formRow">
			<label>Họ tên</label>
			<div class="formRight">
             <input type="text" name="hoten" title="Họ tên khách hàng"   id="hoten" class="tipS validate[required] read" value="<?=@$item['hoten']?>" />
			</div>
			<div class="clear"></div>
		</div>	
         ...
         <div class="formRow">
			...
			<div class="formRight">
             <input type="text" name="dienthoai" title="Số điện thoại khách hàng"   id="dienthoai" class="tipS validate[required] read" value="<?=@$item['dienthoai']?>" /> 
			</div>
			<div class="clear"></div>
		</div>		        
        ...
        <div class="formRow">
			...
			<div class="formRight">
				<div class="myrow">
					<div class="mycol-4 mycol-table-4 mycol-mobi-12">
						<input type="text" name="diachi" title="Địa chỉ khách hàng" id="diachi" class="tipS validate[required] read" value="<?=@$item['diachi']?>" />
					</div>
					<div class="mycol-4 mycol-table-4 mycol-mobi-12 thanhpho_item"><?=get_thanhpho_item();?></div>
					<div class="mycol-4 mycol-table-4 mycol-mobi-12 thanhpho"><?=get_thanhpho();?></div>
				</div>
              <!-- <?php //get_phuong();?>-->
			</div>
			<div class="clear"></div>
		...
		<div class="formRow">
			...
			<div class="formRight">
				<div class="myrow">
					<div class="mycol-3 mycol-mobi-12 mycol-table-4"><?=get_httt();?></div>
				</div>
			</div>
    ...
    <tbody>
        ...
        <tr id="productct<?=$chitietdonhang[$i]['id']?>">
          ...
          <td class="actBtns"><a class="btn btn-danger" original-title="Xóa sản phẩm" href="javascript:del(<?=$chitietdonhang[$i]['id']?>)"><i class="fa fa-times"></i></a></td>
        </tr>
        <?php } ?>
     </tbody>
  </table>
        ...
        <div class="formRow">
			...
			<div class="formRight">
                ...
                <div class="myrow">
					<div class="mycol-3 mycol-mobi-12 mycol-table-4">
