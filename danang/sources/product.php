<?php	if(!defined('_source')) die("Error");
	...
	$urlcu .= (isset($_REQUEST['id_thuonghieu'])) ? "&id_thuonghieu=".addslashes($_REQUEST['id_thuonghieu']) : "";
...
function save_item(){
	...
	if(!kiemtraTenKhongDau('product',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=product&act=man".$urlcu);
	}
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){ 
			$data['photo'] = $photo;	
			//$data['thumb'] = create_thumb($data['photo'], 230, 350, _upload_sanpham,$file_name,1);	
		...
		foreach ($config['lang'] as $key => $value) {
			...
			$data['them'.$key] = magic_quote($_POST['them'.$key]);			
		}		
		$d->setTable('product');
		$d->setWhere('id', $id);
		if($d->update($data))
...
function save_loai(){
	...
	$tenkhongdau = (!empty($_POST['link_url']))?trim($_POST['link_url']):changeTitle($_POST['ten']);
	if(!kiemtraTenKhongDau('product_item',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=product&act=man_item".$urlcu);
	}
...
function save_cat(){
	...
	$tenkhongdau = (!empty($_POST['link_url']))?trim($_POST['link_url']):changeTitle($_POST['ten']);
	if(!kiemtraTenKhongDau('product_cat',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=product&act=man_cat".$urlcu);
	}
...
function save_list(){
	...
	$tenkhongdau = (!empty($_POST['link_url']))?trim($_POST['link_url']):changeTitle($_POST['ten']);
	if(!kiemtraTenKhongDau('product_list',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=product&act=man_list".$urlcu);
	}
...
function save_danhmuc(){
	...
	$tenkhongdau = (!empty($_POST['link_url']))?trim($_POST['link_url']):changeTitle($_POST['ten']);
	if(!kiemtraTenKhongDau('product_danhmuc',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=product&act=man_danhmuc".$urlcu);
	}
	...
	if($id){		
		...
		if($logo = upload_image("file1", _format_duoihinh, _upload_sanpham,$file_name)){
			$data['logo'] = $logo;	
		...
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
		$data['noibat'] = isset($_POST['noibat']) ? 1 : 0;
		$data['noibat_tc'] = isset($_POST['noibat_tc']) ? 1 : 0;
		$data['ngaysua'] = time();
		$data['title'] = magic_quote($_POST['title']);
		$data['h2'] = magic_quote($_POST['h2']);
		$data['keywords'] = magic_quote($_POST['keywords']);
