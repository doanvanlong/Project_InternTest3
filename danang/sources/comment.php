...
function get_items(){
	...
	if($_REQUEST['key']!='')
	{
		$where.=" and (ten like '%".$_REQUEST['key']."%' or email like '%".$_REQUEST['key']."%')";
	}
...
function save_item(){
	...
	if($id){
		...
		$data['dadoc'] = isset($_POST['dadoc']) ? 1 : 0;
		$data['chungnhan'] = isset($_POST['chungnhan']) ? 1 : 0;
		$data['stt'] = $_POST['stt'];
		$data['ngaysua'] = time();
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_binhluan.$row['photo1']);
				delete_file(_upload_binhluan.$row['photo2']);
				delete_file(_upload_binhluan.$row['photo3']);
			}
		...
		} redirect("index.php?com=comment&act=man".$urlcu);} else transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man".$urlcu);
}
