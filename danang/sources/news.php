...
function get_items(){
	...
	if($_SESSION['login']['com'] != 'admin'){
		$where.=" and id_user = '".$_SESSION['login']['id']."'";
	}
...
function get_item(){
	...
	if($_SESSION['login']['com'] != 'admin' && $item['id_user'] != $_SESSION['login']['id']){
		transfer("Bạn Không có quyền vào đây. Vui lòng liên hệ admin. Cảm ơn!",'index.php?com=user&act=admin_edit&type=');
	}
}
//===========================================================
function save_sanpham_team($id)
{
		global $d;
		$idSanpham = $_POST['sanpham'];
		$sql = "delete from #_news_sanpham_condition where id_news='".$id."'";
		$d->query($sql);
		$d->reset();
		for($i=0;$i<count($idSanpham);$i++)
		{
			$sanpham_team['id_news'] = $id;
			$sanpham_team['id_product'] = $idSanpham[$i];
			$d->setTable('news_sanpham_condition');
			$d->insert($sanpham_team);
		}
}
function save_item(){ 
	...
	$tenkhongdau = (!empty($_POST['link_url']))?trim($_POST['link_url']):changeTitle($_POST['ten']);

	if(!kiemtraTenKhongDau('news',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=news&act=man".$urlcu);
	}
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh ,_upload_tintuc,$file_name)){
			$data['photo'] = $photo;
			//$data['thumb'] = create_thumb($data['photo'], 280, 200, _upload_tintuc,$file_name,2);	
			$d->setTable('news');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
		...
		$file_banner_name = $_FILES['file1']['name'];
		if($banner = upload_image("file1", _format_duoihinh ,_upload_tintuc,$file_banner_name)){
			$data['banner'] = $banner;
			$d->setTable('news');
			$d->setWhere('id', $id);
			$d->select();
		...
		$data['tenkhongdau'] = trim($tenkhongdau);
		$data['stt'] = $_POST['stt'];
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
		$data['noibat'] = isset($_POST['noibat']) ? 1 : 0;
		$data['tieubieu'] = isset($_POST['tieubieu']) ? 1 : 0;
		$data['ngaydang'] = strtotime($_POST['ngaydang']);
		$data['ngaysua'] = time();		
		$data['tag'] = $_POST['tag'];
		...
		if($d->update($data))	
		{	
			save_sanpham_team($id);
			mysqli_query("DELETE FROM table_protag where id_pro = '$id'");
			if(trim($_POST['tag'])!=''){
			  ...
			  foreach ($arrTags as $tag)
			  {
				 ...
				 {
					  mysqli_query("insert into table_tags(ten,type) values ('$tag','$type')");
					  $idTag = mysql_insert_id();
				 }
			  
				  //Insert dữ liệu vào table Articles_Tags
				  mysqli_query("insert into table_protag(id_pro,id_tag) values ($id, $idTag)");
				  }
			  }
			}
		...
		$data['stt'] = $_POST['stt'];
		$data['tag'] = $_POST['tag'];
		$data['id_user'] = isset($_SESSION['login']['id']) ? $_SESSION['login']['id'] : 0;
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
		$data['noibat'] = isset($_POST['noibat']) ? 1 : 0;
		$data['tieubieu'] = isset($_POST['tieubieu']) ? 1 : 0;
		$data['ngaydang'] = strtotime($_POST['ngaydang']);
...
function save_cat(){
	...
	if(!kiemtraTenKhongDau('news_cat',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=news&act=man_cat".$urlcu);
	}
...
function save_list(){
	...
	if(!kiemtraTenKhongDau('news_list',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=news&act=man_list".$urlcu);
	}
	if($id){		
		$id =  themdau($_POST['id']);
		if($photo = upload_image("file", _format_duoihinh, _upload_tintuc,$file_name)){
...
function save_danhmuc(){
	...
	$tenkhongdau = (!empty($_POST['link_url']))?trim($_POST['link_url']):changeTitle($_POST['ten']);
	if(!kiemtraTenKhongDau('news_danhmuc',$tenkhongdau,$id)){
		transfer("Link Url này đã tồn tại! Bạn phải thay đổi để website lấy đúng dữ liệu!","index.php?com=news&act=man_danhmuc".$urlcu);
	}
	if($id){		
		$id =  themdau($_POST['id']);
		if($photo = upload_image("file", _format_duoihinh, _upload_tintuc,$file_name)){
		...
		if($photo1 = upload_image("file1", _format_duoihinh, _upload_tintuc,$file_name)){
			$data['icon'] = $photo1;	
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		...
		$sql = "select id,thumb,photo from #_news_danhmuc where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0)
		{
			while($row = $d->fetch_array())
